...
class Quantifier: #quantify .tag.bam file
    ...
    def __init__(self, file_ambiv_gps, file_expression_model=None): #agi: ambivalence group index
        self.model(file_ambiv_gps, file_expression_model)
    ...
    def model(self, file_ambiv_gps, file_expression_model):
        ...
        if not file_expression_model is None:
            expression_model=self._load_expression_model_sf(file_expression_model)

            #compute the probabilities
            self.ambivalence_proba, self.tx_LUT, self.tx_ix_dict = Quantifier._compute_proba(ambiv_gps_tx_LUT, expression_model)
        else:
            self.ambivalence_proba, self.tx_LUT, self.tx_ix_dict = Quantifier._compute_proba(ambiv_gps_tx_LUT, {})

    def quant(self, qbam_in, todf=True, simplify=True, drop_zeros=True):
        # dumps a quantification table
        #prepares the output table for know tx, and an empty dict for unknown stuff
        q=np.zeros((len(self.tx_ix_dict),6))
        q_unknown={}
        q_unknown2={}
        ...
        while not(r==None):
            ...
            ambiv_gene=True if abs(ambiv_ix_gene)>0 else 
            offset=0 if ambiv_ix>0 else 3 #if negative ix, means there is other alignment
            ...
            if ambiv_ix_abs>1: #this is an ambivalence group
                ...
                if Etotal>0:
                    ...
                    for i, tx_ix in enumerate(tx_ixs):
                        q[tx_ix,0+offset]+=Es[i]/Etotal
                else:
                    n_ambiv_without_prior+=1
                    for i, tx_ix in enumerate(tx_ixs):
                        q[tx_ix,1+offset]+=Es[i]/len(tx_ixs)
            elif r.has_tag("ar"):
        ...
        while not(r==None):
            ...
            if ambiv_ix_abs>1: #this is an ambivalence group
                ...
                if Etotal>0: #it has prior proba
                    ...
                    for ii, rec in enumerate(records):
                        ...
                        if i_sampled in ambiv_ix_within:
                            ...
                            break
                    if rec_sampled_ix>-1:
                        records[rec_sampled_ix].set_tag("ap",Es[i_sampled]/Etotal, value_type='f')
                    ...
                    for ii, rec in enumerate(records):
                        ...
                        if i_sampled in ambiv_ix_within:
                            ...
                            break
                    if rec_sampled_ix>-1:
                        records[rec_sampled_ix].set_tag("ap",-1.0/len(i), value_type='f')
                        out.write(records[rec_sampled_ix])
                    else:
                        print("Warning: not able to pick tx for sampling")                    
                ...
                if len(records)>1:
                    ...
                    records[0].set_tag("ap",0.0, value_type='f')
                out.write(records[0])
    ...
    @staticmethod
    def _compute_proba(ambiv_gps_LUT, expression_model, tx_ix_dict={}): 
